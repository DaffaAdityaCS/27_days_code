...
entphone = Entry(root, text='phone', font=('none 13 bold'), textvar = textinn)
...
def updateContact():
    ...
    cursor.execute("UPDATE people SET na,e = ? WHERE phone = ?", (nam, ph))
    connnt.commit()
...
def drop():
    connnt = sqlite3.connect('mysq.db')
    cursor = connnt.cursor()
    cursor.execute("DROP table poeple")
    connnt.commit()
